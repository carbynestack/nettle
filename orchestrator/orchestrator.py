...
class Orchestrator(model_training_pb2_grpc.ModelTrainingServicer):
    ...
    def __init__(self, number_of_clients: int, number_of_rounds: int):
        self._number_of_clients = DEFAULT_NUMBER_OF_CLIENTS if number_of_clients is None else max(1, number_of_clients)
        self._number_of_rounds = DEFAULT_NUMBER_OF_ROUNDS if number_of_rounds is None else max(1, number_of_rounds)
        log(DEBUG, "Orchestrator initialized expecting %d client(s) to connect for %d round(s) of training.",
            self._number_of_clients, self._number_of_rounds)
    ...
    def TrainModel(self, request: model_training_pb2.TrainModelParameters, context):
        ...
        fl.server.start_server(config=fl.server.ServerConfig(num_rounds=self._number_of_rounds), strategy=strategy)
        training_result = model_training_pb2.TrainModelResult(finalModelSecretId=strategy.amphora_model_id)
...
def serve(number_of_clients: int, number_of_rounds: int):
    ...
    model_training_pb2_grpc.add_ModelTrainingServicer_to_server(Orchestrator(number_of_clients, number_of_rounds), server)
