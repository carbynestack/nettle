...
1. The model owner starts the Gramine
   [secret provisioning][gramine-secret-provisioning] service which exposes and
   endpoint that can be used by clients to get attested and to receive the
   secret, which is credentials for accessing the CS instance hosting the
   aggregation logic.
1. The client uses Gramine secret provisioning via secret injection prior to
   ...
   attestation fails the credentials are not injected and the client does not
   start. If someone starts a regular non-protected Flower client, the client
   still can connect to the Flower server but will be unable to fetch the model
   from Amphora as it is unable to authenticate itself.‚
